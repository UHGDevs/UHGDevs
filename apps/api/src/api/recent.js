...
class Recent {
  ...
  static async call(options) {
    ...
    let recent;
    try { recent = await client.callHypixel.get('recentgames', {params: { key: apikey, uuid: uuid }}).then( n => n.data ) } catch (e) {return {success: false, type: "recent", reason: 'Hypixel RecentGames API is getting touble!'}};
