...
exports.uhg_refresh = async (uhg, member, api, guilda) => {
  ...
  if (api.stats) {
    ...
    for (let stat in uhg.dc.cache.bRoles) {
      ...
      } else if (stat == 'Duels' || stat == 'Arena' || stat == 'Walls' || stat == 'VampireZ' || stat == 'Blitz') {
        let staty = api.stats[stat.toLowerCase()]
        let wins = staty.wins
    ...
    let remove = member._roles.filter(n => (uhg.dc.cache.split.badges.includes(n) && !upRole.filter(i => i.id == n).length) || uhg.dc.cache.newBadges.filter(a => a.id == n).size)
