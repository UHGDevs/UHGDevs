...
module.exports = async (uhg, message) => {
  ...
  if (command) {
    ...
    let msg = await command.run(uhg, {username: user.nickname||content.split(" ")[0], args:content.toLowerCase().replace(content.split(" ")[0].toLowerCase(), "").trim()||"", nickname:content.split(" ")[1]||user.nickname||"KOkasfneplatne", verify_data: user}) || "error v posilani mc commandu z discordu"
    if (mcchat) {
      let mcchannel = "/go "
