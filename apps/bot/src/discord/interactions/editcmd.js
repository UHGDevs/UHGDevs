...
try {
  ...
  /* -- API fetching -- */
  let api = interaction.message.api || await uhg.api.call(img.name.split('_')[0], ['hypixel', 'guild', 'friends', 'online'], { premium: true})
  ...
  if (!interaction.message.rotate) interaction.message.rotate = 0
  ...
  /* -- rotate OPTIONS if length is > 25 -- */
  else if (action == 'set' && arg == 'rotate'){
    interaction.message.rotate = interaction.message.rotate >= Math.ceil(data.fields.length / 25) -1 ? 0 : interaction.message.rotate + 1
  }
  ...
  const but1 = new MessageActionRow()
    .addComponents(new MessageButton().setCustomId(`ECMD_${type}_set_rotate`).setStyle('SECONDARY').setEmoji('<:rotate:1012443366421696512>'))
  ...
  let badgeOptions = data.fields.map(e => {return {label: e.name, value: e.stat, emoji: null, default: e.stat == interaction.message.stat}}).chunk(25)
  let selectmenu = new MessageSelectMenu().setCustomId(`ECMD_${type}_set`).addOptions(badgeOptions[interaction.message.rotate])
