module.exports = {
    ...
    run: async (uhg, pmsg) => {
      try{
        ...
        else if (mode === "game") message  = `**${ctfancy} Tourney**: **${api.username}** - ${uhg.f(ctourney.wins)}Wins ${uhg.f(ctourney.wlr)}WLR ${uhg.f(ctourney.kdr)}KDR | Playtime: ${Math.floor(ctourney.playtime/60/60)}h | Most Played Kit: ${uhg.capitalize(ctourney.mostusedkit)}`
        return message
      } catch (e) {
