module.exports = {
    ...
    run: async (uhg, pmsg) => {
      try{
        ...
        let message = `**${year} SummerEvent**: **${api.username}** - Level ${Math.floor(api.hypixel.seasonal.events.summer[year].level || 0)} | ${uhg.f(api.hypixel.seasonal.events.summer[year].xpleft || 0)} XP do dalšího Levelu | ${uhg.f(api.hypixel.seasonal.silver || 0)} Silver`
        return message
      } catch (e) {
